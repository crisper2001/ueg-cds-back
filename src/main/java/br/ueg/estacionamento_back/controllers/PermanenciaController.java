...
public class PermanenciaController {
    ...
    public ResponseEntity<Object> create(@Valid @RequestBody PermanenciaDTO permanenciaDTO) {
        try {
            Optional<FuncionarioModel> funcionario = funcionarioService.validateFuncionarioExists(permanenciaDTO.getFuncionarioId());
    ...
    public ResponseEntity<Object> updateById(@PathVariable Long id, @Valid @RequestBody PermanenciaDTO permanenciaDTO) {
        try {
            Optional<PermanenciaModel> permanencia = permanenciaService.updateById(id, permanenciaMapper.toPermanenciaModel(permanenciaDTO));
            if (permanencia.isPresent()) {
                return ResponseEntity.ok(permanencia);
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("");
